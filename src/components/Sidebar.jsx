  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b">
        <h2 className={`text-xl font-bold text-indigo-600 ${collapsed ? 'hidden' : 'block'}`}>
          TaskMaster
        </h2>
        <button onClick={() => setCollapsed(!collapsed)} className="text-gray-500 hover:text-indigo-500">
          <Menu size={20} />
        </button>
      </div>
        ...
        {menuItems.map(({ name, icon: Icon, path }) => (
          <Link
            ...
            className={`flex items-center gap-3 px-3 py-2 rounded-md hover:bg-indigo-50 transition ${
              location.pathname === path ? 'bg-indigo-100 text-indigo-600 font-medium' : ''
            }`}
          >
            <Icon size={20} />
